...
async function gciFetch(url) {
  for (const proxy of PROXIES) {
    try { const r = await fetch(proxy(url)); if (r.ok) return await r.json(); } catch {}
  }
  // Direct attempt as last resort
  try { const r = await fetch(url); if (r.ok) return await r.json(); } catch {}
  return null;
}
async function lookupTaxId(tid){const d=await gciFetch(`${GCIS_BASE}?$format=json&$filter=Business_Accounting_NO eq ${tid}&$skip=0&$top=1`);if(d?.length)return{company:d[0].Company_Name,address:d[0].Company_Location||""};return null}
async function lookupName(name){const d=await gciFetch(`${GCIS_BASE}?$format=json&$filter=Company_Name like ${name}&$skip=0&$top=5`);if(Array.isArray(d))return d.map(x=>({company:x.Company_Name,taxId:x.Business_Accounting_NO,address:x.Company_Location||""}));return[]}
